...
######################################## DASHBOARD ########################################
    ...
    with st.container():
        ...
        with st.expander('RESUMEN DE POZOS'):
            ...
            map_pozos_loc = px.scatter_mapbox(well_sum, lat="Latitud", hover_name='Pozo', lon="Longitud", zoom=12, color='Estado')
            map_pozos_loc.update_layout(title='Ubicación geográfica de los pozos Sitio Grande', mapbox_style="stamen-terrain", margin={"r":0,"t":100,"l":0,"b":0}, legend=dict(orientation="h", yanchor="bottom", y=1, xanchor="right", x=0.9), showlegend=True, autosize=True)
            map_pozos_loc
            well_sum
        ...
        with st.expander('GRÁFICOS DE PRODUCTIVIDAD'):
            ...
            w_prod_plot.update_layout(title_text = f'HISTÓRICO DE PRODUCCIÓN {filt_pozos}', hovermode="x unified", font=dict(family="sans-serif", size=10, color="black"), legend=dict(orientation="h", yanchor="bottom", y=1, xanchor="right", x=1), margin={"r":0,"t":100,"l":100,"b":0}, height=370, width=1020)
            ...
            well_prod_press.update_layout(hovermode="x unified", margin={"r":0,"t":100,"l":0,"b":0}, xaxis=dict(title_text="<b>Año</b>", nticks=25, domain=[0.07, 0.93]),
                ...
                yaxis3=dict(nticks=15, exponentformat='none', title="Gas [MMPCPD]", titlefont=dict(color="black", size=12), tickfont=dict(color="black", size=9),
                        ...
                        side="right",
                        position = 0.93),
                yaxis4=dict(nticks=15,exponentformat='none', title="Presión [Kg/cm2]", titlefont=dict(color="black", size=12), tickfont=dict(color="black", size=9),
                        anchor="free",
                        overlaying="y",
                        side="right",
                        position=1))
            ...
            raa_rga_plot.update_layout(title_text=f'RGA/RAA {filt_pozos}', height=350, width=1020, font=dict(family="sans-serif", size=10, color="black"), legend=dict(orientation="h", yanchor="bottom", y=1, xanchor="right", x=1))
            raa_rga_plot.update_layout(hovermode="x unified", margin={"r":0,"t":100,"l":100,"b":0}, xaxis=dict(title_text="<b>Año</b>", nticks=25, domain=[0, 0.95]),
                yaxis=dict(nticks=15, exponentformat='none', title="RGA [sm3/sm3]", titlefont=dict(
            ...
            cum_oil_plot.update_layout(font=dict(family="sans-serif", size=10, color="black"), hovermode="x unified", title=f'Producción de Aceite Acumulado {filt_pozos}', legend=dict(orientation="h", yanchor="bottom", y=1, xanchor="right", x=1), margin={"r":0,"t":100,"l":0,"b":0}, height=300, width=350)
            ...
            cum_water_plot.update_layout(font=dict(family="sans-serif", size=10, color="black"), hovermode="x unified", title=f'Producción de Agua Acumulada {filt_pozos}', legend=dict(orientation="h", yanchor="bottom", y=1, xanchor="right", x=1), margin={"r":0,"t":100,"l":0,"b":0}, height=300, width=350)
            ...
            cum_gas_plot.update_layout(font=dict(family="sans-serif", size=10, color="black"), hovermode="x unified", title=f'Producción de Gas Acumulado {filt_pozos}', legend=dict(orientation="h", yanchor="bottom", y=1, xanchor="right", x=1), margin={"r":0,"t":100,"l":0,"b":0}, height=300, width=350)
            ...
            st.caption("Qo Inicial: " + str(round(pozo['Aceite_bpd'].iloc[0],1)))
            st.caption("Qo Final: " + str(round(pozo['Aceite_bpd'].iloc[-1],1)))
            st.caption("Qw Inicial: " + str(round(pozo['Agua_bpd'].iloc[0],4)))
            st.caption("Qw Final: " + str(round(pozo['Agua_bpd'].iloc[-1],4)))
            st.caption("Qg Inicial: " + str(round(pozo['Gas_mmcfpd'].iloc[0],4))) 
            st.caption("Qg Final: " + str(round(pozo['Gas_mmcfpd'].iloc[-1],4)))
            ...
            st.caption("Producción acumulada de gas: " + str(round(pozo['Gas_mmcfpm'].sum(),2)) + " Millones de Pies Cúbicos")
                ...
                #w_prod_plot.write_html(f'{filt_pozos} Histórico de Producción.html', auto_open=True)
            #if st.button(f'Exportar Gráfico de RAA-RGA Histórica {filt_pozos}') == True:
                #raa_rga_plot.write_html(f'{filt_pozos} WOR-GOR-WC% historico.html', auto_open=True)
